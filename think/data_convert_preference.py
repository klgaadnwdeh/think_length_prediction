...
# 加载原始数据
def convert_kto(path="../data/think/"):
    # 加载原始数据
    data = load_from_disk(path)

    # 获取文件名并添加前缀"kto_"
    filename = os.path.basename(path)  # 获取最后一个名称
    temp_path = "kto_" + filename
    ...
    for row in tqdm(data, total=len(data), desc="Processing Data"):
        ...
        true_row = {
            ...
            'completion': [{'content': think_label_content, 'role': 'assistant'}],
            'label': True
        }
        ...
        transformed_data.append(false_row)
...
if __name__=='__main__':
    ...
    if function_choice==0:
        convert_kto(train_data_path)
        convert_kto(valid_data_path)
